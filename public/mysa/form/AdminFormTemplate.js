// JavaScript Document
define(["dojo/_base/declare", "dojo/on", "dojo/dom", "dojo/dom-construct", "dojo/query", "dojo/string", "dojo/request/xhr", "dojo/dom-form", "dojo/request", "dojo/json",
...
function(declare, on, dom, domConstruct, query, string, xhr, domForm, request, JSON,
...
return declare(null, {
	...
	try{
		...
		var td = query("#" + this.form.id + " td");
		for(var i = 0; i < this.items.length; i++){
			this.items[i].general.place = td[2*i];
			var label = new Label(this.items[i]);
			label.make();
			this.items[i].general.place = td[2*i+1];
			var dijit = new Dijit(this.items[i]);
			dijit.make();
		}
		if(this.form.recaptcha == true){
			domConstruct.place("recap-label", td[td.length-2]);
			domConstruct.place("recap", td[td.length-1]);
		}
		}
	catch(e){
