				...
				//IF HAS A SUBMENU....
					...
					for(var j = 0; j < menuType.menu[i].submenu.length; j++){
						...
						//HAS A SUBMENU...
							...
							//START CREATING SUBMENU ITEMS TO ATTACH TO SUBMENU 
							for(var k = 0; k < menuType.menu[i].submenu[j].submenu.length; k++){
								//IF NO SUBMENU OPTIONS...
								if(menuType.menu[i].submenu[j].submenu[k].submenu == null){
									var href = this.makeHref(menuType.menu[i].submenu[j].submenu[k]);
									var menuItem = new MenuItem({
										label: menuType.menu[i].submenu[j].submenu[k].label,
										"data-mysa-prop": { href: href, },
									});
									on(menuItem, "click", function(){
										window.location.href = this["data-mysa-prop"].href;
									});
									subsubMenu.addChild(menuItem);
								}
								//HAS A SUBMENU...
								else{
									//var subSubMenu = new Menu({ style: "display: none;"});
							...
							subMenu.addChild(new PopupMenuItem({	label: menuType.menu[i].submenu[j].label,
																popup: subsubMenu
															}));		
						}
					}
