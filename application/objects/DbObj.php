...
class Object_DbObj
{
	...
	*/
	...
	private function _setSearch($search){
		$this->_search = $search;
	}
		...
		try {
			...
			switch($count){
				case 1:
					...
					unset($general[0]['id']);
					$this->_setObjGeneral($general[0]);
					$this->_setData();
					break;
				case 0:
					$this->_setObjId(NULL);
					$this->_setObjGeneral(NULL);
					$this->_setObjData(NULL);
					break;
				default:
					$this->_setObjId(NULL);
					$this->_setObjGeneral(NULL);
	...
	private function _setData(){
		$rMethod = new Zend_Reflection_Method(__METHOD__);
		try {
			$this->_setObjData($this->_queryObjData());
		} catch (Exception $e) {
			$this->_errorHandler->_setError($this->_reflectionClass, $rMethod, $e);
		}
	...
	public function _setPayload($search = NULL){
		if($search != NULL){
		...
		if($this->_search == NULL){
			...
			$this->_setGeneral();
			//THIS GETS DONE IN _setGeneral() IF IT IS SUCCESSFUL
			//$this->_setData();
	...
	*/	
	...
	public function _getGenDataItem($item){
		return $this->_objPayload['general'][$item];
	}

	public function _getData($getWhat = NULL){
		$data = $this->_getObjData();
	...
	public function _getDataCategory($category, $getWhat = NULL ){
		$value = array();
		foreach($this->_getObjData() as $row){
	...
	*/	
	...
	private function _getObjId(){
		return $this->_objPayload['id'];
	}
	
	private function _getObjGeneral(){
		return $this->_objPayload['general'];
	}

	private function _getObjData(){
		return $this->_objPayload['data'];
	}
	...
	private function _setObjGeneral($general){
		$this->_objPayload['general'] = $general;
	}

	private function _setObjData($data){
		$this->_objPayload['data'] = $data;
	}

	public function _refresh(){
		return $this->_setPayload();
	}
