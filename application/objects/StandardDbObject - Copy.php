	...
	*/
	...
	private function _queryObjGeneral(){
		...
		try{
			...
			if(is_numeric($this->_objPayload['id']) && $this->_objPayload['id'] > 0){
				$select->where('id = ?', $this->_objPayload['id']);
			}
			else{
	...
	private function _queryObjData(){
		...
		try{
			...
			$select->where('ref_id = ?', $this->_objPayload['id']);
	...
	private function _setObjGeneral(){
		...
		try {
			...
			switch($count){
				case 1:
					$this->_objPayload['id'] = $general[0]['id'];
					unset($general[0]['id']);
					$this->_objPayload['general'] = $general[0];
					$this->_setObjData();
					break;
				case 0:
					$this->_objPayload['id'] = NULL;
	...
	public function _updateData($updates){
		...
		try{
			foreach($updates as $update){
				$where = array();
				$where['ref_id = ?'] = $this->_objPayload['id'];
				...
				switch($count){
					case 1:
						...
						break;
					case 0:
						$update['ref_id'] = $this->_objPayload['id'];
